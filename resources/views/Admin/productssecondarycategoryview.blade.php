        ...
        <section id="app" style="margin-top: 10px;" class="content">
            <div class="row">
                <div class="col-sm-12">
                    ...
                    @endif
                    <div class="card">
                        <div class="card-header">Products Seconary Category View</div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-sm-7">
                                    <form action="#" method="">
                                        ...
                                        <div class="form-group">
                                            <label for="PrimaryCategoryId">Blog Category</label>
                                            <select class="form-control{{$errors->has('PrimaryCategoryId') ? ' is-invalid' : ''}}" id="PrimaryCategoryId" name="PrimaryCategoryId">
                                                <option value="" selected disabled>=============Select Blog Category===========</option>
                                                @foreach(App\ProductsPrimaryCategory::all() as $cat)
                                                    <option value="{{$cat->id}}" {{$cat->id == $Category->PrimaryCategoryId ? 'selected="selected"' : ''}}>{{$cat->CategoryName}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control{{$errors->has('SeoHeading') ? ' is-invalid' : ''}}" value="{{$Category->SeoHeading}}" id="SeoHeading"  name="SeoHeading" placeholder="Enter Seo Heading" readonly="readonly">
                                        </div>
                                        <div class="form-group">
                                            <label for="CategoryDetails">Category Details</label>
                                            <textarea type="text" class="form-control{{$errors->has('CategoryDetails') ? ' is-invalid' : ''}}" readonly="readonly" id="CategoryDetails" name="CategoryDetails" placeholder="Enter Category Details....">{{$Category->CategoryDetails}}</textarea>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <a href="{{url('admin/products-secondary-category')}}" class="btn btn-primary">BACK</a>
                                    </form>
                                </div>
                                <div class="col-sm-5">
                                    <table id="mytable" class="table table-striped">
                                        ...
                                        <tr>
                                            <th scope="col">#</th>
                                            <th scope="col">Primary Cate. Name</th>
                                            <th scope="col">Secondary Cate. Name</th>
                                            <th scope="col">Action</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        @foreach(App\ProductsSecondaryCategory::all() as $Category)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{$Category->primaryCategory->CategoryName}}</td>
                                                <td>{{$Category->CategoryName}}</td>
                                                <td>
                                                    <a href="{{url('admin/view-products-secondary-category',[$Category->id])}}" class="btn btn-success"><i style="font-size:17px;" class="fa fa-eye"></i></a>
                                                    <a href="{{url('admin/edit-products-secondary-category',[$Category->id])}}" class="btn btn-info"> <i style="font-size:17px;" class="fa fa-edit"></i></a>
                                                    <a href="{{url('admin/delete-products-secondary-category',[$Category->id])}}" class="btn btn-danger" onclick="return ConfirmDelete();" ><i class="fas fa-trash-alt"></i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                        </tbody>
